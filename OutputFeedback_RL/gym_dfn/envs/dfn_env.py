...
class DFN(discrete.DiscreteEnv):
	...
	def __init__(self, sett, cont_sett, init_v=3.2, init_t=p['T_amb']):
		...
		p['D_s_n0'] = np.random.uniform(2.5e-15, 3.5e-13) # Diffusion coeff for solid in neg. electrode, [m^2/s]
		p['D_s_p0'] = np.random.uniform(2.24e-15, 3.5e-13) # Diffusion coeff for solid in pos. electrode, [m^2/s]
	...
	def step(self, action):
		...
		try:
			...
			is_error = True
			raise Exception('Numerical error occured. Check the code.')
			# pdb.set_trace()
		...
		reward = r_step + r_temp + r_volt + r_etas
		self.episode_step += 1
